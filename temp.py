...
from utility import spp_algorithm_list, plot_convergence
...
supported_sp_algorithms = Literal["GLC", "LC", "LS", "LSF"]
...
class LinkBased:
    def __init__(
            self,
            network: Network,
            sp_alg: supported_sp_algorithms
    ):
        self.network = network
        self.sp_alg = spp_algorithm_list[sp_alg]
        self.sp_so_alg = spp_algorithm_list["LS_SO"]
        self.convergence_model: ConvergenceCheck = ConvergenceCheck(self.network)

    # Method of successive average
    def run_MSA(self, model_type, main_gap, verbose=True):
        pass
    ...
    def run_FW_UE(
            ...
            verbose: bool = True,
            step_up: float = 1,
            step_down: float = 1,  # self-regulated method
    ) -> (list[float], float):
        s = pc()
        self.network.reset_flow_and_cost()
        self._all_or_nothing()
        for link in self.network.link_set[1:]:
            link.flow = link.auxiliary_flow  # 1 get initial solution
        current_gap, gap_list = inf, list()
        while current_gap > main_gap:  # main loop
            self.network.update_all_link_costs()  # 2 update cost
            self._all_or_nothing()  # 3 find steepest descent direction
            step = self._bisection(bisection_gap)  # 4 find optimal step
            step = min(step * step_up, 1) if len(gap_list) >= 2 and current_gap < gap_list[-2] else step * step_down
            self._update_flow(step)  # 5 update flow
            current_gap = self.convergence_model.run()  # 6 convergence check
            gap_list.append(current_gap)
            if verbose:
                print(f'iteration {len(gap_list)}: gap = {current_gap}')
        e = pc()
        print(f"TSTT={self.network.get_TSTT()}, running time={e-s:.2f}s, total iterations={len(gap_list)}")
        return gap_list, e-s

    def run_FW_SO(self, main_gap, bisection_gap=1e-4, verbose=True) -> list[float]:
        pass
    ...
    def run_MSA_UE(
        ...
        while current_gap > main_gap:  # main loop
            self.network.update_all_link_costs()  # 2 update cost
            self._all_or_nothing()  # 3 find steepest descent direction
            step = 1 / (len(gap_list) + 2) # 4 find optimal step
            step = min(step * step_up, 1) if len(gap_list) >= 2 and current_gap < gap_list[-2] else step * step_down
            self._update_flow(step)  # 5 update flow
            current_gap = self.convergence_model.run()  # 6 convergence check
            gap_list.append(current_gap)
    ...
    def run_MSA_SO(self, main_gap, verbose=True):
        pass

    def _update_flow(self, step: float) -> None:
        for link in self.network.link_set[1:]:
            link.flow = link.flow + step * (link.auxiliary_flow - link.flow)

    def _all_or_nothing(self) -> None:
        for link in self.network.link_set[1:]:
            link.auxiliary_flow = 0
        for od in self.network.od_pair_set:
            ori, dest, demand = od.origin, od.destination, od.demand
            shortest_links = self.sp_alg(self.network, ori.node_id, dest.node_id)
    ...
    def _bisection(
        ...
        left, right, mid, max_iter_times, iter_times = 0, 1, 0.5, 500, 1
        while abs(derivative_f(mid)) > bisection_gap:
            iter_times += 1
            ...
            elif derivative_f(mid) * derivative_f(right) > 0:
                right = mid
            else:
                left = mid
            mid = (right + left) / 2
...
class PathBased:
    ...
    # Gradient Projection
    def run_GP(self, model_type="UE", ssp_alg="LS", main_gap=1e-4, bisection_gap=1e-4, verbose=True):
        pass

    # Manifold suboptimization
    def run_MS(self, model_type="UE", ssp_alg="LS", main_gap=1e-4, bisection_gap=1e-4, verbose=True):
        pass
...
if __name__ == "__main__":
    ...
    link_model = LinkBased(sf, "LS")
    gap_list_regulated, _ = link_model.run_MSA_UE(
        main_gap=1e-4,
        bisection_gap=1e-4,
        verbose=True,
        step_up=1.9,
    ...
    gap_list_traditional, _ = link_model.run_MSA_UE(
        main_gap=1e-4,
        bisection_gap=1e-4,
        verbose=False,
        step_up=1,
        step_down=1)
    plot_convergence({(1.9, 0.6): gap_list_regulated, (1, 1): gap_list_traditional})
